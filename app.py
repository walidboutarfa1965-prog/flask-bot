...
total_trades = 0
winning_trades = 0
settings = {'default_quantity': 0.001, 'symbols': ['BTCUSDT', 'ETHUSDT'], 'broker': 'Binance', 'funding_company': 'None'}
    ...
    <div class="section">
        ...
        <form method="POST" action="/update">
            <div class="flex">
                ...
                <label>الوسيط: <select name="broker"><option value="Binance">Binance</option><option value="Bybit">Bybit</option><option value="IBKR">IBKR</option></select></label>
                <label>شركة التمويل: <input type="text" name="funding" value="{{ funding_company }}" placeholder="FTMO"></label>
...
def index():
    ...
    return render_template_string(HTML, balance=f"{balance:.2f}", total_trades=total_trades, win_rate=win_rate,
                                  ...
                                  funding_company=settings['funding_company'])

@app.route('/update', methods=['POST'])
def update():
    settings['default_quantity'] = float(request.form.get('quantity', 0.001))
    settings['broker'] = request.form.get('broker', 'Binance')
    settings['funding_company'] = request.form.get('funding', 'None')
    return index()
